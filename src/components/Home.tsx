...
const Home = () => {
  ...
  // plans Stateを管理できるStateを初期化する。初期値はからの配列:[]
  ...
  // JavaScriptは処理が終わっていなくても次のコードを実行してしまうと言う特徴がある。
  const onFormSubmit = async (event: { preventDefault: () => void; }) => {
    try{
      // formタグを使うとボタンを押した際にデフォルトでonSubmitイベントが走る。
      // 今回はonSubmitイベントが走ったら自分で作ったonFormSubmit関数を実行したいので
      // こちらの記述でデフォルトのsubmit処理をキャンセルする
      ...
      // onFormSubmitが実行され、市場にAPIのレスポンスが返ってきたら、plans Stateに更新される。
          ...
          <div className="field">
            ...
            <select className="ui dropdown"
             ...
             >
              ...
              <option value="16000">20,000円</option>
            </select>
          </div>
          <div className="field">
